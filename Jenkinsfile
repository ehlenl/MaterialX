                    ...
                    stage("Create Nuget Packages") {
                        ...
                        withEnv(properties) {
                            if(axisNodeValue.contains("GEC-vs")) {
                                ...
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx-content.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop adsklibdir=%WORKSPACE%\\adsk-git-submodules\\adsklib -Prop win_compiler=${win_compiler}
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx-sdk_win_intel64.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop adsklibdir=%WORKSPACE%\\adsk-git-submodules\\adsklib -Prop win_compiler=${win_compiler}
                                """
                                ...
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx-content.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop adsklibdir=$WORKSPACE/adsk-git-submodules/adsklib -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
