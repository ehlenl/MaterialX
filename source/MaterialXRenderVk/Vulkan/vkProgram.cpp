...
// OpenGL Constants
unsigned int VkProgram::UNDEFINED_OPENGL_RESOURCE_ID = 0;
int VkProgram::UNDEFINED_OPENGL_PROGRAM_LOCATION = -1;
int VkProgram::Input::INVALID_OPENGL_TYPE = -1;
...
unsigned int VkProgram::build()
{
    ...
    {
        throw ExceptionRenderError("Glsl Vulkan program creation error", errors);
    }
...
void VkProgram::bindAttribute(const VkProgram::InputMap& inputs, MeshPtr mesh)
{
    if (!mesh)
    {
    ...
    for (const auto& input : inputs)
    {
        ...
        if (_attributeBufferIds.find(input.first) == _attributeBufferIds.end())
        {
            ...
            unsigned int bufferId = VkProgram::UNDEFINED_OPENGL_RESOURCE_ID;
...
void VkProgram::bindPartition(MeshPartitionPtr part)
{
    ...
    else
    {
        ...
        unsigned int indexBuffer = VkProgram::UNDEFINED_OPENGL_RESOURCE_ID;
...
void VkProgram::bindMesh(MeshPtr mesh)
{
    ...
    VkProgram::InputMap foundList;
    const VkProgram::InputMap& attributeList = getAttributesList();
    ...
    const VkProgram::InputMap& uniformList = getUniformsList();
...
ImagePtr VkProgram::bindTexture(unsigned int uniformType, int uniformLocation, const FilePath& filePath,
                                  ImageHandlerPtr imageHandler, const ImageSamplingProperties& samplingProperties)
{
        ...
        uniformType >= GL_SAMPLER_1D && uniformType <= GL_SAMPLER_CUBE)
    {
        ...
        {
            VkTextureHandlerPtr textureHandler = std::static_pointer_cast<VkTextureHandler>(imageHandler);
...
MaterialX::ValuePtr VkProgram::findUniformValue(const string& uniformName, const VkProgram::InputMap& uniformList)
{
...
void VkProgram::bindUniformLocation(int location, ConstValuePtr value)
{
    if (_programId == UNDEFINED_OPENGL_RESOURCE_ID)
    {
        throw ExceptionRenderError("Cannot bind without a valid program");
...
void VkProgram::clearInputLists()
{
    _uniformList.clear();
    ...
    if (_shader)
    {
        ...
        for (const auto& uniformMap : ps.getUniformBlocks())
        {
            ...
            for (size_t i = 0; i < uniforms.size(); ++i)
            {
                ...
                // Ignore types which are unsupported in Glsl Vulkan.
        ...
        {
            throw ExceptionRenderError("Glsl Vulkan uniform parsing error", errors);
        }
    }
...
int VkProgram::mapTypeToOpenGLType(const TypeDesc* type)
{
    ...
    return VkProgram::Input::INVALID_OPENGL_TYPE;
}
...
const VkProgram::InputMap& VkProgram::updateAttributesList()
{
    ...
    if (_shader)
    {
        ...
        if (uniformTypeMismatchFound)
        {
            throw ExceptionRenderError("Glsl Vulkan attribute parsing error", errors);
        }
    }
...
void VkProgram::findInputs(const string& variable,
